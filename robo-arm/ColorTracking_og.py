...
# The angle of closing when the clamp clamp is taken
servo1 = 500
...
#Set the RGB lamp color of the expand plate so that it is consistent with the colors to track
def set_rgb(color):
    if color == "red":
...
def move():
    ...
    #     # Different color wooden placement coordinates(x, y, z)
    #     coordinate = {
    ...
    #                 setBuzzer(0.1)               
    #                 result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0) # Do not fill in the run time parameters, adaptive runtime
    #                 if result == False:
    #                     unreachable = True
    #                 else:
    #                     unreachable = False
    ...
    #                 first_move = False
    #                 action_finish = True
    #             elif not first_move and not unreachable: # Not the first time I detected objects
    #                 set_rgb(detect_color)
    #                 if track: # If it is the tracking phase
    #                     if not __isRunning: # Stop and exit sign bit detection
    #                         continue
...
def run(img):
    ...
    # def frame_adjustments(): 
    frame_resize = cv2.resize(img_copy, size, interpolation=cv2.INTER_NEAREST)
    frame_gb = cv2.GaussianBlur(frame_resize, (11, 11), 11)


    ####3
    #If an object recognized in an area is detected, the area has been detected until there is not
    ...
    area_max = 0
    areaMaxContour = 0
    if not start_pick_up: # if not in pick up mode
